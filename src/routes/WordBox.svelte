<script lang="ts">
	...
	function startTimer() {
		...
		}, 100); // update about every 0.1 second
	}
		...
		if (words.length > 0) {
			userTypedWords.forEach((word: String, index: number) => {
				...
				for (let i = 0; i < word.length; i++) {
					if (words[index][i] && word[i].toUpperCase() === words[index][i].toUpperCase()) userCorrect++;
					else userIncorrect++;
				}
			});
		}
	...
	function handleKeyPress(value: any): any {
		if (value == 'Space') {
			...
			if (userInput.length > words[currentWordIndex].length) {
	...
	function compareInputAndWord(value: string) {
		...
		currentWordElem.children[currentWordElem.children.length - 1].innerHTML =
			currentWordElem.children[currentWordElem.children.length - 1].innerHTML.slice(1);
		currentWordElem.insertBefore(
			span,
			currentWordElem.children[currentWordElem.children.length - 1]
		);
	}
	function replaceSpanWithText() {
		...
		let currentWordElem = wordsElem!.children[currentWordIndex];
		currentWordElem.children[currentWordElem.children.length - 1].innerHTML =
			words[currentWordIndex][charIndex] +
			currentWordElem.children[currentWordElem.children.length - 1].innerHTML;
...
<div id="wordContainer" class="word-container">
	...
	<div
		class="stat-bar"
		style="max-height:28px; min-height:28px; border: 1px solid pink; width: 100%; display: flex;"
	>
	...
	{#if !isDone}
		<input
			...
			id="userInput"
			style="width:99%; text-align: center;margin-left: auto;margin-right: auto; margin-bottom:0; margin-top: auto;"
			autocomplete="off"
			disabled
		/>
	{:else}
